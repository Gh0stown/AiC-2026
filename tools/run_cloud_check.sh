...
mkdir -p "$WS/.verify"
export ROS_MASTER_URI=http://localhost:11890
export GAZEBO_MASTER_URI=http://localhost:11900
roslaunch --port=11890 competition_robot robot_gazebo.launch gui:=false rviz:=false > "$WS/.verify/cl.log" 2>&1 & SIM=$!
sleep 40
timeout 40 python3 "$WS/tools/check_cloud.py" 2>/dev/null | sed 's/^/  /'
