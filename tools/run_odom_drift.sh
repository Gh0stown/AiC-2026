...
mkdir -p "$WS/.verify"
export ROS_MASTER_URI=http://localhost:11930
export GAZEBO_MASTER_URI=http://localhost:11940
roslaunch --port=11930 competition_robot robot_gazebo.launch gui:=false rviz:=false > "$WS/.verify/od.log" 2>&1 & SIM=$!
sleep 40
timeout 60 python3 "$WS/tools/check_odom_drift.py" 2>/dev/null | sed 's/^/  /'
