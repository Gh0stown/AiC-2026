...
**2026-09-18 新发现的隐患 —— 用 YOLO 做识别前必须先解决**：

当前"亮/灭"是靠灯珠**沿轴滑动**表示的：亮 = 推到 `x=+0.05`（露在灯箱外），
灭 = 缩到 `x=-0.03`（**完全藏进灯箱内部**，灯箱 x 跨度 ±0.04）。
而 `base` 上那 3 个透镜视觉（`lens_red` / `lens_yellow` / `lens_green`）
材质全是**近黑色 `0.03 0.03 0.03`**，不是各自的颜色。

于是相机看到的画面是："黑灯箱上有 3 个黑圆，其中某一个位置是个彩色亮圆"。
**用这种数据训 YOLO，模型学到的是「彩色圆在竖排第几个位置」，而不是「哪个灯在发光」。**
真机上三个透镜始终可见、只有亮度差别 → 模型很可能直接失效（sim2real 断裂）。

而且这个隐患对**经典 OpenCV 方案反而无所谓**（它只看颜色），
所以选 YOLO 就必须先修这个。

修法（等官方物料到了、和换贴图一起做）：

- 把 `lens_red / lens_yellow / lens_green` 的材质改成**各自的深色**，而不是统一近黑：
  暗红 `0.25 0.02 0.02`、暗黄 `0.25 0.18 0.0`、暗绿 `0.02 0.25 0.04`
- 改完仿真画面就等同于真实红绿灯（**三个有色透镜 + 一个亮的**），
  训练数据的域与真机一致，YOLO 学到的才会是"亮度"而不是"位置"
- 同时 `tools/check_traffic_light.py` 的分类逻辑要相应调整
  （目前只统计高饱和像素，亮度差变小后阈值要重标）
