...
实测修复后：z 稳定在 0，位置两次测量 0.0307 → 0.0294（不再漂），
但会**稳定地比配置值内移约 3 cm**（y 从 2.00 落到 1.97）——这是落地的正常沉降。
支架仍在车道边缘（1.94）之外。
...
两盏都朝**场地内侧**（上灯 −y、下灯 +y），即朝着各自那条车道。

**为什么不按示意图的橙色箭头把上灯朝 +x**：横排灯宽 64 cm，朝 +x 摆在顶部
墙边时灯箱沿 y 铺开 64 cm（从 y=2.00 算会到 2.32），而内墙在 2.076、墙高
0.5 m，灯箱高度正好被墙挡住——**绿色透镜在靠墙那侧，会被完全遮住**。
朝内侧摆进深只有 8 cm，灯面平行于墙，既不挡车道也不被墙遮。
（下灯的橙箭头朝 +y，本来就与"朝内侧"一致。）

贴墙放（|y| ≈ 2.00，内墙 2.076）的原因：车道中心在 y ≈ ±1.70，车体半宽
0.24 m，车体边缘到 1.94 / −1.98；灯箱在 0.34 m 以上**高于车体**，只有 2.5 cm
细支架在车高范围内，所以贴墙不会撞车——但把 64 cm 宽的灯横着摆进车道就会挡路。
