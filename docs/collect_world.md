...
* **红绿灯**：`tl_top` / `tl_bot` 各三种状态，亮的灯珠分别是 **左=红 / 中=黄 / 右=绿**，
  灭的是各自的暗色版本 —— 一眼可辨，说明"强制灯态"那条链是通的 ✓
* **车牌**：三辆车正对车牌，字符清晰（`苏A·B8Q62` / `鄂D·7B5Q2` / `苏A·PL12A`）✓
  也顺带验证了车的朝向（`yaw=π` → 车牌朝 +x）没设错

> 建议红绿灯用 `--light-dists 1.0,1.4,1.8`（比赛里红灯点位就是 1.0 m；
> 0.8 m 时灯箱顶部会贴到画面边缘）；车牌保持 0.6/0.9/1.2（覆盖比赛的 0.7 m）✓
