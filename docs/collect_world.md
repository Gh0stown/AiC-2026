...
![红绿灯与车牌例图](collect_lights_plates.png)

* **红绿灯（1.0 m）**：`tl_top` / `tl_bot` 各三种状态，亮的灯珠分别是
  **左=红 / 中=黄 / 右=绿**，灭的是各自的暗色版本 —— 一眼可辨，
  说明"强制灯态"那条链是通的 ✓；**灯箱完整在画面内**（三颗灯珠 + 双腿 + 箱体都见）✓
* **车牌（0.9 m）**：三辆车车牌清晰（`苏A·B8Q62` / `鄂D·7B5Q2` / `苏A·PL12A`），
  **HyperLPR3 端到端 9/9 全对**（0.6/0.9/1.2 m 三个距离各一张都读对）✓

### 两个已修的坑（都出在"朝向/距离"上）

1. **车牌拍不到**：车牌在车体局部 **+x** 面，而采集车放在 +x 侧 ——
   车必须 `yaw=0`。最初抄了比赛 world spawn 的 `3.1416`，结果车牌朝 -x，
   拍到的全是车尾、例图里根本没有车牌 ✗。现在 `CAR_YAW=0.0` ✓
2. **灯箱拍不全**：0.8 m 时灯箱顶部只剩 76 px 就贴到画面边缘了 ✗。
   拍全需要 `d ≥ 0.68 m`，舒服要 `≥1.0 m` → 红绿灯默认改成
   **`--light-dists 1.0:2.0:0.25`**（距离扫描 5 档，就是"让车在前后一段距离里
   来回移动收集多种情况"），并加了 `--frame-margin` 守卫：**灯箱/车牌必须完整在画面内**
   才采这个位姿，否则换距离/重抽。

> 另外 `--light-dists` / `--car-dists` 现在都支持 **`起:止:步长`** 写法
> （如 `1.0:2.0:0.25`），就是一个距离扫描。
